...
class IndexAwareCircularBuffer<T extends IndexedItem> {
  ...
  void _moveChild(int fromIndex, int toIndex) {
    ...
    if (child == null) {
      // Source slot is empty — nothing to move
      _array[toCyclicIndex] = null;
    } else if (child.attached) {
      child._move(toIndex);
      _array[toCyclicIndex] = child;
    } else {
      // Source exists but detached — re-attach instead of crashing
      dev.log('CircularBuffer: re-attaching detached item at $fromIndex→$toIndex',
          name: 'xterm.buffer');
...
mixin IndexedItem {
  ...
  void _move(int newIndex) {
    if (!attached) {
      dev.log('CircularBuffer: _move called on detached item, newIndex=$newIndex',
          name: 'xterm.buffer');
      return;
    }
