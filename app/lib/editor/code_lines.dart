/// code_lines.dart — Segmented collection of [CodeLine]s.
///
/// Provides [CodeLines], a segmented list that stores code lines in
/// fixed-size [CodeLineSegment] chunks for efficient insertion, deletion,
/// and copy-on-write semantics. Also provides index-to-line-index
/// conversion methods that account for folded chunks.

part of editor;

const int _kCodeLineSegamentDefaultSize = 256;

/// A segmented, copy-on-write collection of [CodeLine]s.
///
/// Stores lines in fixed-size [CodeLineSegment] chunks (default 256 lines
/// each) so that insertions and deletions only clone the affected segment
/// rather than the entire list. Provides index-to-line-index conversion
/// that accounts for folded chunks.
class CodeLines {
...
/// A fixed-size segment of [CodeLine]s within a [CodeLines] collection.
///
/// When [dirty] is true, the segment is a shared reference and must be
/// cloned before mutation (copy-on-write).
