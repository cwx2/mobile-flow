/// _code_field.dart — Custom RenderObject for painting and laying out code.
///
/// Contains [_CodeFieldRender], the core render object that handles painting
/// code lines, the cursor, selection highlights, and find-match overlays.
/// Also performs hit testing to map pointer positions to text offsets, and
/// manages the virtual viewport so only visible lines are laid out.
part of editor;
...
class _CodeFieldRender extends RenderBox implements MouseTrackerAnnotation {
  ...
  _CodeFieldRender({
    ...
    _backgroundRender = _CodeFieldExtraRender(
      painters: [
        ...
        _CodeBracketMatchPainter(selectionColor),
      ]
    );
  ...
  set selection(CodeLineSelection value) {
    ...
    // Update bracket match highlight when cursor moves
    _backgroundRender.find<_CodeBracketMatchPainter>().match =
        _CodeBracketMatchPainter.findMatch(_codes, value);
    markNeedsLayout();
  }
...
/// Bracket match result: two positions that form a matching pair.
class _BracketMatchResult {
  final int openLine;
  final int openOffset;
  final int closeLine;
  final int closeOffset;

  const _BracketMatchResult({
...
/// Paints a semi-transparent background behind matched bracket pairs.
///
/// When the cursor is adjacent to a bracket character, this painter
/// highlights both the bracket at the cursor and its matching partner.
/// Uses the same color as the selection highlight for visual consistency.
class _CodeBracketMatchPainter extends _CodeFieldExtraPainter {

  Color _color;
  _BracketMatchResult? _match;

  _CodeBracketMatchPainter(this._color) : _match = null;
  ...
  set match(_BracketMatchResult? value) {
  ...
  void paint(Canvas canvas, Size size, _CodeFieldRender render) {
    final m = _match;
    if (m == null) return;
    if (_color == Colors.transparent || _color.alpha == 0) return;

    final paint = Paint()..color = _color;

    // Draw highlight on the opening bracket
    _drawBracketRect(canvas, render, m.openLine, m.openOffset, paint);
    // Draw highlight on the closing bracket
    _drawBracketRect(canvas, render, m.closeLine, m.closeOffset, paint);
  }

  /// Draw a background rectangle behind a single bracket character.
  void _drawBracketRect(
    Canvas canvas,
    ...
    final paragraph = render.findDisplayParagraphByLineIndex(lineIndex);
    if (paragraph == null) return;

    final rects = paragraph.paragraph.getRangeRects(
      TextRange(start: charOffset, end: charOffset + 1),
    );
    if (rects.isEmpty) return;

    final lineOffset = paragraph.offset - render.paintOffset;
    for (final rect in rects) {
      canvas.drawRRect(
        RRect.fromRectAndRadius(rect.shift(lineOffset), const Radius.circular(2)),
        paint,
      );
    }
  }
  ...
  ///
  /// Checks the character before and after the cursor. If either is a
  /// bracket, searches for its matching partner using a stack-based scan.
  /// Returns null if no bracket is adjacent to the cursor or no match found.
  static _BracketMatchResult? findMatch(CodeLines codes, CodeLineSelection selection) {
    if (!selection.isCollapsed) return null;

    final lineIndex = selection.extentIndex;
    if (lineIndex < 0 || lineIndex >= codes.length) return null;

    final lineText = codes[lineIndex].text;
    final offset = selection.extentOffset;

    // Check character before cursor
    if (offset > 0) {
      final result = _tryMatch(codes, lineIndex, offset - 1, lineText[offset - 1]);
      if (result != null) return result;
    }
    // Check character after cursor
    if (offset < lineText.length) {
      final result = _tryMatch(codes, lineIndex, offset, lineText[offset]);
      if (result != null) return result;
    }
    return null;
  }
  ...
  /// Try to match a single character at the given position.
  static _BracketMatchResult? _tryMatch(
    CodeLines codes, int lineIndex, int charOffset, String char,
  ) {
    final partner = _pairs[char];
    if (partner == null) return null;

    if (_openers.contains(char)) {
      // Search forward for closing bracket
      return _searchForward(codes, lineIndex, charOffset, char, partner);
    } else {
      // Search backward for opening bracket
      return _searchBackward(codes, lineIndex, charOffset, char, partner);
    }
  }

  /// Search forward from the given position for a matching closing bracket.
  /// Skips brackets inside string literals (single and double quotes).
  static _BracketMatchResult? _searchForward(
    CodeLines codes, int startLine, int startOffset,
    String open, String close,
  ) {
    int depth = 0;
    for (int line = startLine; line < codes.length; line++) {
      final text = codes[line].text;
      final start = (line == startLine) ? startOffset : 0;
      int quoteState = 0; // 0=normal, 1=in single quote, 2=in double quote
      // Pre-scan from line start to determine quote state at our start position
      if (line == startLine && start > 0) {
        quoteState = _quoteStateAt(text, start);
      }
      for (int i = start; i < text.length; i++) {
        final ch = text[i];
        // Track string literal boundaries
        if (quoteState == 0) {
          if (ch == "'" && !_isEscaped(text, i)) { quoteState = 1; continue; }
          if (ch == '"' && !_isEscaped(text, i)) { quoteState = 2; continue; }
        } else if (quoteState == 1 && ch == "'" && !_isEscaped(text, i)) {
          quoteState = 0; continue;
        } else if (quoteState == 2 && ch == '"' && !_isEscaped(text, i)) {
          quoteState = 0; continue;
        }
        // Skip brackets inside strings
        if (quoteState != 0) continue;
        if (ch == open) {
          depth++;
        } else if (ch == close) {
          depth--;
          if (depth == 0) {
            return _BracketMatchResult(
              openLine: startLine, openOffset: startOffset,
              closeLine: line, closeOffset: i,
            );
          }
        }
      }
  ...
  /// Search backward from the given position for a matching opening bracket.
  /// Skips brackets inside string literals (single and double quotes).
  static _BracketMatchResult? _searchBackward(
    CodeLines codes, int startLine, int startOffset,
    ...
    for (int line = startLine; line >= 0; line--) {
      final text = codes[line].text;
      final start = (line == startLine) ? startOffset : text.length - 1;
      // Pre-scan from line end to determine quote state at our start position
      // (scanning backward, so we check state from the right)
      int quoteState = 0;
      if (line == startLine && start < text.length - 1) {
        quoteState = _quoteStateAt(text, start + 1);
      }
      for (int i = start; i >= 0; i--) {
        final ch = text[i];
        // Track string literal boundaries (scanning backward: closing quote
        // enters string, opening quote exits string)
        if (quoteState == 0) {
          if (ch == "'" && !_isEscaped(text, i)) { quoteState = 1; continue; }
          if (ch == '"' && !_isEscaped(text, i)) { quoteState = 2; continue; }
        } else if (quoteState == 1 && ch == "'" && !_isEscaped(text, i)) {
        ...
        if (text[i] == close) {
          ...
          if (depth == 0) {
            return _BracketMatchResult(
              openLine: line, openOffset: i,
              closeLine: startLine, closeOffset: startOffset,
            );
          }
        }
      }
  ...
  /// Determine the quote state at a given position by scanning from line start.
  /// Returns 0=normal, 1=inside single quote, 2=inside double quote.
  static int _quoteStateAt(String text, int upTo) {
    int state = 0;
    for (int i = 0; i < upTo && i < text.length; i++) {
      final ch = text[i];
      if (state == 0) {
        if (ch == "'" && !_isEscaped(text, i)) state = 1;
        else if (ch == '"' && !_isEscaped(text, i)) state = 2;
      } else if (state == 1 && ch == "'" && !_isEscaped(text, i)) {
        state = 0;
      } else if (state == 2 && ch == '"' && !_isEscaped(text, i)) {
        state = 0;
      }
    }
    return state;
  ...
  /// Check if the character at [index] is preceded by a backslash escape.
