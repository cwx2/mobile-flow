/// code_chunk.dart — Code folding (chunk) API.
///
/// Provides [CodeChunkController] (fold/expand state management),
/// [CodeChunk] (a foldable line range), [CodeChunkAnalyzer] (abstract
/// analyzer), [DefaultCodeChunkAnalyzer] (brace-matching analyzer), and
/// [NonCodeChunkAnalyzer] (no-op analyzer that disables folding).

part of editor;

/// Controller that manages code folding state for the editor.
///
/// Listens to [CodeLineEditingController] changes and re-analyzes foldable
/// regions via [CodeChunkAnalyzer] in a background isolate. The current
/// list of [CodeChunk]s is exposed through [ValueNotifier.value].
...
/// A foldable region spanning from line [index] to line [end] (exclusive).
///
/// When [canCollapse] is true, the lines between [index]+1 and [end]-1 can
/// be hidden. [collapseSize] returns the number of lines that would be hidden.
class CodeChunk {

  /// The starting line index of the foldable region.
  final int index;

  /// The ending line index of the foldable region (exclusive).
  final int end;
...
/// Interface for analyzing code lines to discover foldable regions.
///
/// Implement this to provide language-specific folding rules.
/// See [DefaultCodeChunkAnalyzer] for the built-in brace-matching
/// implementation and [NonCodeChunkAnalyzer] to disable folding entirely.
abstract class CodeChunkAnalyzer {

  /// Analyze [codeLines] and return all foldable [CodeChunk] regions.
  List<CodeChunk> run(CodeLines codeLines);

}

/// A no-op analyzer that disables code folding entirely.
///
/// Pass this to [CodeEditor.chunkAnalyzer] to turn off fold indicators.
...
/// Default chunk analyzer that detects foldable regions by matching
/// balanced bracket pairs: `()`, `[]`, `{}`.
///
/// Handles string literals (single and double quotes) and escape characters
/// to avoid false matches inside strings. Works for most C-style languages
/// but may not suit indentation-based languages like Python.
...
/// A bracket symbol found during chunk analysis, tagged with its line [index].
