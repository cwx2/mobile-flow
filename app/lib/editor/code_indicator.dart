/// code_indicator.dart — Line number and code fold indicator widgets.
///
/// Provides [DefaultCodeLineNumber] (line number gutter widget),
/// [DefaultCodeChunkIndicator] (fold/expand arrow widget), and
/// [CodeChunkIndicatorPainter] (abstract painter for fold arrows).
...
/// Signature for a builder that creates the indicator column (line numbers,
/// fold arrows, etc.) displayed to the left of the editor content.
...
/// Snapshot of the currently visible code line paragraphs and the focused
/// line index, used by indicator widgets to paint in sync with the editor.
class CodeIndicatorValue {

  /// The rendered paragraphs currently visible in the viewport.
  final List<CodeLineRenderParagraph> paragraphs;

  /// The index of the line that currently has the cursor, or -1 if none.
...
/// A leaf render widget that paints line numbers in the indicator gutter.
///
/// Reads visible paragraphs from [notifier] and paints the corresponding
/// line numbers, highlighting the focused line with [focusedTextStyle].
...
/// A leaf render widget that paints fold/expand arrows in the indicator gutter.
///
/// Shows a collapse arrow (▼) for foldable regions and an expand arrow (▶)
/// for already-folded regions. Tapping an arrow toggles the fold state via
/// [CodeChunkController].
...
/// Interface for painting fold/expand indicator arrows.
///
/// Implement this to customize the appearance of the fold indicators
/// in [DefaultCodeChunkIndicator].
abstract class CodeChunkIndicatorPainter {

  /// Paint the collapse indicator (shown when a region can be folded).
  void paintCollapseIndicator(Canvas canvas, Size container);

  /// Paint the expand indicator (shown when a region is already folded).
  void paintExpandIndicator(Canvas canvas, Size container);

}

/// Default painter that draws small triangular arrows for fold indicators.
