/// code_line.dart — Core data model for the code editor.
///
/// Defines [CodeLineEditingController] (the main editing controller),
/// [CodeLineEditingValue] (immutable editing state snapshot),
/// [CodeLineSelection] (multi-line selection range), [CodeLine] (single line
/// of code with optional folded chunks), and supporting types like
/// [CodeLinePosition], [CodeLineRange], and [CodeLineOptions].

part of editor;

/// Signature for a builder that wraps or replaces the [TextSpan] of a single
/// code line before it is painted.
...
///
/// Wraps another [CodeLineEditingController] and forwards all calls to it,
/// allowing subclasses to override specific behaviors without replacing
...
/// A single line of source code, optionally containing folded [chunks].
///
/// Each [CodeLine] holds its [text] content and a list of child [chunks]
/// that represent lines hidden by code folding. When [chunks] is non-empty,
/// this line acts as a fold header (see [chunkParent]).
class CodeLine {
...
/// Some options controlling line break style and indentation.
...
/// A pair of (line index, chunk index) identifying a code line in the
/// potentially folded document.
class CodeLineIndex {

  /// The visible line index in the editor.
  final int index;

  /// The index within the parent line's [CodeLine.chunks], or -1 if this
  /// is a top-level line.
...
/// Layout information for a single rendered code line.
///
/// Combines the [IParagraph] text layout with its [offset] in the editor
/// viewport and metadata about folding state.
...
/// Line break style used when joining code lines into text.
enum TextLineBreak {
