...
class GitHandler(BaseHandler):
    ...
    async def handle_git_status_all(self, client_id, ws, msg):
        ...
        """
        logger.info(f"查询所有仓库状态: client={client_id[:8]}...")
    ...
    async def handle_git_commit(self, client_id, ws, msg):
        ...
        logger.info(f"提交: repo={Path(payload.repo).name}, message={payload.message[:50]}")
        ...
        result = await manager.run(
            Op.Commit,
            run_operation=lambda: git.commit(payload.message),
    ...
    async def handle_git_push(self, client_id, ws, msg):
        ...
        logger.info(f"推送到远程: repo={Path(repo_path).name}")
    ...
    async def handle_git_pull(self, client_id, ws, msg):
        ...
        logger.info(f"拉取远程更新: repo={Path(repo_path).name}")
    ...
    async def handle_git_checkout(self, client_id, ws, msg):
        ...
        logger.info(f"切换分支: repo={Path(repo_path).name}, branch={payload.branch}")
        result = await manager.run(
            Op.Checkout,
    ...
    async def handle_git_repos(self, client_id, ws, msg):
        ...
        logger.info(f"发现 {len(repos)} 个仓库")
        await self.send(ws, Message.from_typed(
